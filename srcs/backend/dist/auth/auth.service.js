...
try { }
    ...
    async;
    sendAuthorizationCodeRequest(code, string);
    {
        const requestBody = {
    ...
    async;
    sendUserInfoRequest(token, string);
    {
    ...
    async;
    createUser(userInfo, any, res, Response);
    Promise < client_1.User > {
...
generateTwoFASecret(userId, number);
{
    secret: string;
    otpauthUrl: string;
}
...
async;
verifyTwoFACode(userId, number, code, string);
Promise < boolean > {
    const: user, User: client_1.User
} | null;
