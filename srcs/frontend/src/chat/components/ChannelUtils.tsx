  ...
  try {
    ...
    const fetchChannelHeaders = listChannelId.map(async (id: string) => {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/chat/getChannelHeader?channelId=${channelId}`,
        GetRequestOptions
      );
...
export const getNumberUsersInChannel = async (
  channelId: number
): Promise<number> => {
  try {
    const response: Response = await fetch(
      `${API_BASE_URL}/api/chat/getNumberUsersInChannel?channelId=${channelId}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
...
export const getChannelName = async (
  ...
  try {
    const response: Response = await fetch(
      `${API_BASE_URL}/api/chat/getChannelName?channelId=${channelId}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
...
function compareUsersWithNumbers(
  users: User[],
  participants: number[]
): boolean {
...
export const isChannelExist = async (
  ...
  try {
    ...
    for (const convId of channelList) {
      const response = await fetch(
        `${API_BASE_URL}/api/chat/getUsersFromChannelId?channelId=${convId}`,
        GetRequestOptions
      );
      handleHTTPErrors(response, {});
...
export function isChannelIsLive(
  channelId: number,
  userId: number,
  socket: Socket
): Promise<boolean> {
...
export const setHeaderNameWhenTwoUsers = async (
  channelId: string,
  userId: number,
  socket: Socket
): Promise<isChannelNameConnected | null> => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/chat/getUsersFromChannelId?channelId=${channelId}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
    const users: User[] = await response.json();
    if (!users) throw new Error("Error fetching database");
    if (users.length < 2) {
      return channelInfo;
    }
...
export const leaveChannel = async (
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/chat/leaveChannel`,
      requestOptions
    );
    handleHTTPErrors(response, {});
...
export const getUsernamesBySubstring = async (
  userIdCaller: number,
  substring: string
): Promise<User[]> => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/chat/getUsernamesFromSubstring?substring=${cleanSubstring}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
    const listUsers: User[] = await response.json();
    const filteredListUsers = listUsers.filter(
      (user: User) => user.id !== userIdCaller
    );
...
export const getUsernamesInChannelFromSubstring = async (
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/chat/getUsernamesInChannelFromSubstring?channelId=${channelId}&substring=${cleanSubstring}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
    const users: User[] = await response.json();
...
export const banUserList = async (
  userList: User[],
  channelId: number,
  socket: Socket
): Promise<void> => {
  try {
    for (const user of userList) {
      ...
      const response: Response = await fetch(
        `${API_BASE_URL}/api/chat/banUserFromChannel`,
        {
          method: "POST",
          credentials: "include",
          headers: {
...
export const kickUserList = async (
  userList: User[],
  channelId: number,
  socket: Socket
) => {
  for (const user of userList) {
    try {
      const targetId: number = user.id;
      const response: Response = await fetch(
        `${API_BASE_URL}/api/chat/kickUserFromChannel`,
        {
          method: "POST",
          credentials: "include",
          headers: {
...
export const fetchChannelAdmins = async (
  channelId: number
): Promise<User[]> => {
  try {
    const response: Response = await fetch(
      `${API_BASE_URL}/api/chat/getAdmins?channelId=${channelId}`,
      GetRequestOptions
    );
    if (!response.ok) {
...
export const fetchUserAdminTable = async (
  channelId: number
): Promise<{ user: User; isAdmin: boolean; updated: boolean }[]> => {
  try {
    const users: User[] = await fetchChannelUsers(channelId);
    const admins: User[] = await fetchChannelAdmins(channelId);

    const userAdminTable: { user: User; isAdmin: boolean; updated: boolean }[] =
      users.map((user) => ({
        user,
...
export const manageAdminsToChannel = async (
  userList: { user: User; isAdmin: boolean }[],
  channelId: number,
  inviterId: number
): Promise<void> => {
  try {
    for (const user of userList) {
      ...
      if (user.isAdmin === true) {
        const response = await fetch(
          `${API_BASE_URL}/api/chat/addAdminToChannel`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/chat/removeAdminFromChannel`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
export const addUserIdToChannel = async (
  channelId: number,
  userId: number
): Promise<number> => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/chat/addUserToChannel`,
      requestOptions
    );
    if (!response.ok) {
      return Promise.reject(await response.json());
...
export const addUserListToChannel = async (
  userList: User[],
  channelId: number,
  socket: Socket
): Promise<Response[]> => {
...
export const isUserIsBan = async (
  channelId: number,
  userId: number
): Promise<boolean> => {
  try {
...
export const joinProtectedChannel = async (
  channelId: number,
  userId: number,
  password: string
): Promise<boolean> => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/chat/addUserToProtectedChannel`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const joinChannel = async (
  channel: IisChannelExist,
  userId: number
): Promise<{ channelType: ChannelType; channelId: number }> => {
  try {
...
export const fetchConversation = async (
  userId: number,
  channelId: number,
  addMsgToFetchedConversation: (message: Message) => void
) => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/chat/getAllMessagesByChannelId?channelId=${channelId}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
    ...
    messageList.map((message: Message) => {
      userId === message.senderId
        ? (message.messageType = "MessageTo")
        : (message.messageType = "MessageFrom");
...
export const manageChannelPassword = async (
  channelId: number,
  channelType: string,
  ...
  try {
    const response: Response = await fetch(
      `${API_BASE_URL}/api/chat/manageChannelPassword`,
      {
        method: "POST",
        credentials: "include",
        headers: {
...
export const manageChannelType = async (
  channelId: number,
  channelType: string
) => {
  try {
...
export const isOwner = async (
  ...
  try {
    const response: Response = await fetch(
      `${API_BASE_URL}/api/chat/isOwner?channelId=${channelId}&userId=${userId}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
...
export const getUsername = async (userId: number): Promise<string> => {
  try {
    const response: Response = await fetch(
      `${API_BASE_URL}/api/users/getPublicName?userId=${userId}`,
      GetRequestOptions
    );
    handleHTTPErrors(response, {});
